...
for D_ in "$(dirname "$0")/../config" "$(dirname "$0")/../../config" /data/scripts/config; do
...
for BK in BOOT_MEM_TUNE BOOT_TRIM_CACHES BOOT_CUT_SERVICES BOOT_SET_NETWORK BOOT_TIME_SYNC BOOT_EXPOSE BOOT_ROTATE_LOGS; do
    BV="$(gv "$BK")"

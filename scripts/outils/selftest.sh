...
main()
{
    ...
    FOUND=0
    for B in "$BASE/core" "$BASE/../core" /data/scripts/core; do
    ...
    check_rc "inspect_proc HELP"   "0" sh "$BASE/inspect_proc.sh" HELP
    check_rc "inspect_dev HELP"    "0" sh "$BASE/inspect_dev.sh" HELP
    check_rc "launcher_toggle HELP" "0" sh "$BASE/launcher_toggle.sh" HELP
    ...
    check_rc "preflight"           "0" sh "$BASE/preflight.sh"
    check_rc "xrun LIST"           "0" sh "$BASE/xrun.sh" LIST

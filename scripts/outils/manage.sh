...
WL_SH=""
for F in "$BASE/disable_wireless.sh" /data/scripts/disable_wireless.sh \
         "$BASE/../optim/disable_wireless.sh"; do
    [ -f "$F" ] && WL_SH="$F" && break
done
...
service_page()
{
    ...
    for TOOL in front_digit motd; do
        TS_=""
        for C in "$BASE/$TOOL.sh" "/data/scripts/$TOOL.sh" "$BASE"/../*/"$TOOL.sh"; do
            [ -f "$C" ] && { TS_="$C"; break; }
        done
        if [ -n "$TS_" ] && sh "$TS_" STATUS > /dev/null 2>&1; then
            ok_ko OK "$TOOL actif"
        else
